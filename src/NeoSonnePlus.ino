...
void setup()
{
  ...
  /*-----------------------------------------------------------------config lesen ------------------------*/
  int err = loadConfig();
  if (err == MY_ERROR || err == MY_ERROR_NO_HTML)
...
void loop()
{
  ...
  {
    int i = 0;
    while (output[actOutputIdx].typ == GPIOPIN && i < cfg.anzoutputs)
    { // GPIOs überspringen
      actOutputIdx = ++actOutputIdx % cfg.anzoutputs;
      i++;
    }
    //---------------------------------------
    if (now > output[actOutputIdx].nextAccessTime)
    { // readtime reached
